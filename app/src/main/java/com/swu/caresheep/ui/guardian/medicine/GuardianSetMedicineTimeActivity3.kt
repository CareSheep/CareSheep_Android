...
class GuardianSetMedicineTimeActivity3 : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_guardian_set_medicine_time3)

        thirdMedcineButton.setOnClickListener {

            Log.d("T", "$countMedicine")
            pushTime()

            dbRef5 = FirebaseDatabase.getInstance().getReference("TakingMedicine").child("2")
            dbRef6 = FirebaseDatabase.getInstance().getReference("TakingMedicine").child("2")

            val updatedData = HashMap<String, Any>()
            updatedData["time"] = "$result1,$result2,$result3"

            dbRef5.updateChildren(updatedData).addOnSuccessListener(){
            ...
            // 데이터 읽어와서 비교하기
            dbRef6.addValueEventListener(object : ValueEventListener {
    ...
    private fun nextPage() {
        ...
        if(countMatched == 1 && nextpage == 1){
            ...
            // 다음 액티비티 이동
            startActivity(Intent(this, GuardianSetMedicineTimeActivity4::class.java))
            countMedicine++ // 카운트 증가
            Log.d("증가3!","카운트 증가했고,$countMedicine")
        }
    }
